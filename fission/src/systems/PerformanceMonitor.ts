import {
  globalAddToast,
  globalOpenModal,
} from "@/components/GlobalUIControls.ts";
...
export class PerformanceMonitoringSystem extends WorldSystem {
  ...
  antiCount: number = 0;
  performanceRegister: boolean = false;
  ...
  public update(_: number) {
    ...
    if (newIsCritical == this.isCritical) {
      ...
      if (this.antiCount > 10 && this.antiCount > 0.5 * this.activeCount) {
        ...
        if (this.isCritical && !this.performanceRegister) {
          this.performanceRegister = true;
